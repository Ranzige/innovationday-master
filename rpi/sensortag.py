...
def Get_Dict(IR_temp_celsius, Ambient_temp_celsius, lux_luminance, rel_humidity, barPressure):
	myData = {
				'IR_temp_celsius': IR_temp_celsius,
...
def main():
	...
	if flag_to_send == True:
		print "INFO: Sending to Database..."
		send_to_DB(barPressure, "barPressure") # Send to database the Barrometric Pressure in hPa (hectoPascal)




	data = Get_Dict(IR_temp_celsius, Ambient_temp_celsius, lux_luminance, rel_humidity, barPressure)
	dataJson = json.dumps(data, sort_keys=True)
	myData = json.loads(dataJson)
	with open('/home/pi/sensorTag/innovationday-master/static/sensorData.json','w') as outfile:
